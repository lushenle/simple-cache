...
当前 Simple-Cache 的缓存数据完全存储在内存中，进程退出后数据丢失。虽然分布式模式下已经具备 Raft WAL + Snapshot 恢复链路，但 `cache.Dump/Load` 仍然是 single 模式下最直接的缓存持久化能力。在以下场景中，缓存持久化是必要的：
...
- 不实现分布式快照同步（分布式恢复依赖 Raft snapshot，不使用 `cache.Load`）
...
持久化模块位于缓存引擎层，与 Cache 结构紧密集成。需要注意：本设计文档描述的是 **Cache Dump/Load**，主要用于 single 模式；distributed 模式的状态恢复由 Raft snapshot + WAL replay 负责。
